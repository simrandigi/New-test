	...
	class YITH_WAPO_Admin {
		...
		public function register_panel() {
			...
			if ( $capability == 'manage_options' ) {
				...
				// $tabs['impexp']		= __( 'Import / Export', 'yith-woocommerce-product-add-ons' );
				// $tabs['help']		= __( 'Help', 'yith-woocommerce-product-add-ons' );
				if ( isset( $_REQUEST['tab'] ) && $_REQUEST['tab'] == 'debug' ) {
		...
		public function admin_enqueue_scripts() {
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'yith_wapo_panel' ) {
				...
				// CSS
				wp_enqueue_style( 'wapo-admin', YITH_WAPO_URL . 'assets/css/_new_yith-wapo-admin.css', false, rand(10000,99999) );

				// JS
				wp_register_script( 'yith_wapo_admin', YITH_WAPO_URL . 'assets/js/_new_yith-wapo-admin.js', array( 'jquery' ), rand(10000,99999), true );
		...
		public function enable_disable_block() {
			global $wpdb;
			$block_id = isset( $_POST['block_id'] ) ? floatval( $_POST['block_id'] ) : 0;
			$block_vis = isset( $_POST['block_vis'] ) ? floatval( $_POST['block_vis'] ) : 0;
			// Update db table
		...
		public function enable_disable_addon() {
			global $wpdb;
			$addon_id = isset( $_POST['addon_id'] ) ? floatval( $_POST['addon_id'] ) : 0;
			$addon_vis = isset( $_POST['addon_vis'] ) ? floatval( $_POST['addon_vis'] ) : 0;
			// Update db table
		...
		public function sortable_blocks() {
			...
			$prev_item = isset( $_POST['prev_item'] ) ? floatval( $_POST['prev_item'] ) : 0;
			$next_item = isset( $_POST['next_item'] ) ? floatval( $_POST['next_item'] ) : 1;
			...
			$wpdb->update( $table, $data, array( 'id' => $moved_item ) );

			echo $moved_item . '-' . $priority;
			// echo  'prev[' . $prev_item . '] next[' . $next_item . '] gap[' . $gap . '] med[' . $med . '] priority[' . $priority . ']';
