...
if ( ! class_exists( 'YITH_WAPO_Install' ) ) {
	...
	class YITH_WAPO_Install {
		...
		public function __construct() {
			...
			// Check migration
			if ( ! $yith_wapo_db_migration || ( isset( $_GET['yith_wapo_force_db_migration'] ) && $_GET['yith_wapo_force_db_migration'] == 1 ) ) {
		...
		public function migration() {
			...
			$old_groups_array = $wpdb->get_results( $query );
			...
			foreach ( $old_groups_array as $key => $block ) {
				...
				foreach ( $old_addons_array as $addon_key => $addon ) {
					...
					//$request['wapo_action'] = 'save-addon';
