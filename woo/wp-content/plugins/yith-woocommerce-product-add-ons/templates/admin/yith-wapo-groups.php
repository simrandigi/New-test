...
<div id="wapo-groups" class="wrap wapo-plugin">
	...
	for ( $visibility = 9; $visibility >=0; $visibility-- ) :
		...
		switch ( $visibility ) {
			case 0: echo '<span class="dashicons dashicons-hidden" style="margin: -1px 5px 0px 0px;"></span> <strong>' . __( 'Hidden groups', 'yith-woocommerce-product-add-ons' ) . '</strong>'; break;
			case 1: echo '<span class="dashicons dashicons-lock" style="margin: -1px 5px 0px 0px;"></span> <strong>' . __( 'Administrators only', 'yith-woocommerce-product-add-ons' ) . '</strong>'; break;
			case 9: echo '<span class="dashicons dashicons-visibility" style="margin: -1px 5px 0px 0px;"></span> <strong>' . __( 'Public groups', 'yith-woocommerce-product-add-ons' ) . '</strong>'; break;
			default: echo '<span class="dashicons dashicons-visibility" style="margin: -1px 5px 0px 0px;"></span> <strong>' . __( 'Public groups', 'yith-woocommerce-product-add-ons' ) . '</strong>'; break;
		}
		...
		<table class="wp-list-table widefat fixed striped posts" style="margin-bottom: 30px;">
			...
			foreach ( $rows as $key => $value ) : ?>
				...
				<tr>
					...
					<td>
						<a href="edit.php?post_type=product&page=yith_wapo_group_addons&id=<?php echo $value->id; ?>">
							<?php echo yith_wapo_get_addons_number_by_group_id( $value->id ) . ' ' . __( 'add-ons', 'yith-woocommerce-product-add-ons' ); ?>
						</a>
					</td>
					<td><?php
	...
	<a href="admin.php?page=yith_wapo_panel&yith_wapo_v2=1" class="button yith-update-button"><?php echo __( 'Switch to the new 2.0 version', YITH_WAPO_LOCALIZE_SLUG ); ?></a><br />
	<small>* <?php echo __( 'You will not lose the current configuration of your add-ons.', YITH_WAPO_LOCALIZE_SLUG ); ?></small>
